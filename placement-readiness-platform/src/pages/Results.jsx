...
import { getAnalysisById, getLatestAnalysis, updateSkillConfidence, formatDate } from '../utils/historyStorage'
import { calculateFinalScore } from '../utils/analysisSchema'
import { 
  Building2, 
  Briefcase, 
  Calendar, 
...
function Results() {
  ...
  useEffect(() => {
    ...
    if (result) {
      ...
      // Use finalScore if available (new schema), otherwise fall back to readinessScore (legacy)
      setLiveScore(result.finalScore ?? result.readinessScore ?? 50)
    } else {
  ...
  // Calculate live score based on skill confidence (uses baseScore for calculation)
  useEffect(() => {
    if (!analysis) return
    
    // Use baseScore (stable) for calculation, or fall back to readinessScore (legacy)
    const baseScore = analysis.baseScore ?? analysis.readinessScore ?? 50
    const newFinalScore = calculateFinalScore(baseScore, skillConfidence)
    
    setLiveScore(newFinalScore)
  }, [skillConfidence, analysis])

  // Toggle skill confidence with score stability
  const toggleSkillConfidence = useCallback((skill) => {
    setSkillConfidence(prev => {
      ...
      // Persist to localStorage with score recalculation
      if (analysis) {
        const newConfidenceValue = newConfidence[skill]
        updateSkillConfidence(analysis.id, skill, newConfidenceValue)
      }
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-center text-gray-500 mt-2">
                Base: {analysis.baseScore ?? analysis.readinessScore ?? 50} | Final: {liveScore}
                {analysis.updatedAt && analysis.updatedAt !== analysis.createdAt && (
                  <span className="block mt-1">Updated: {formatDate(analysis.updatedAt)}</span>
                )}
              </p>
            </CardContent>
          </Card>
